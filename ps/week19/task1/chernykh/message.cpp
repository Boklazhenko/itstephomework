...
bool Message::operator==(const Message &other) const {
    if (strcmp(_msg, other._msg) == 0)
    {
        return _contexts == other._contexts;
    }
...
const char *const Message::toString() const {
    ...
    {
        size += strlen(_contexts[i]->key()) + strlen(_contexts[i]->value()) + 2;
    }
    ...
    {
        strcat(str, _contexts[i]->key());
        strcat(str, _contexts[i]->value());
    }
    return str;
}
